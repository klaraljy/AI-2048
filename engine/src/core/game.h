...
// 加权与纯随机的比例（/1000）。
//
// ⚠️ 困难档从 **750 提到 880**（2026-09-13 第二次修订）。理由：那 25% 的均匀兜底
// 正是用户看到的"还是经常生成在空白处" —— 它的存在是为了防"系统作弊感"，
// 但在"封最大块的路"这个**明确的战术意图**面前，25% 太让步了。
// 实测：提到 88% 后，最大块所在行/列合计拿到 80~87% 的落点概率。
//
// 简单档保持不变：它的偏置是"帮玩家"，帮得温和一些是对的。
inline constexpr std::uint64_t kEasyWeightedShare = 800;
inline constexpr std::uint64_t kHardWeightedShare = 880;

// 权重 = exp(score × strength) 的放大强度（/100）。
//
// ⚠️ 这两个值**不是**文档给的 0.70 / 0.85，而是实测标定出来的。
//
// 历史上这里踩过两次，方向相反，都记下来：
//
// 1. **太强**：评分常数按文档取（加总量级 19.6）再配 0.85，困难档的
//    max/min 权重比达到 **1700 万倍**，25% 的均匀兜底被完全压倒，
//    退化成"必定落同一格"，也就是文档自己警告的「系统作弊感」。
// 2. **太弱**（上面那个问题的过度矫正）：strength 压到 0.35 之后，
//    贴最大块只有 +200 分，被"拥挤度 +120×多个邻居"和"断裂点 +150"盖过 ——
//    实测**最大块所在的行/列完全没有被优待**（42.8% vs 均匀 42.9%）。
//    用户直接反馈："生成在空白位置的概率还是比较高"。
//
// 最终定在 **硬档 0.60 / 加权占比 880**，而且**先把评分结构改成
// "与最大块同行/同列"为主导项**（见 HostileScore），再调这两个旋钮 ——
// 结构不对时调强度只是在放大一个次要项。
//
// 实测（同一盘面，紧贴最大块的格子，均匀应占 14.29%）：
//
//     strength 0.35 / 750  →  30.8%
//     strength 0.50 / 850  →  40.2%
//     strength 0.60 / 880  →  **45.6%**   ← 采用
//     strength 0.70 / 900  →  50.4%
inline constexpr std::int32_t kEasyWeightStrength = 35;  // 0.35
inline constexpr std::int32_t kHardWeightStrength = 60;  // 0.60

/**
 * 评分参与指数运算前的**饱和上限**（百分数）。
 *
 * 为什么需要它：`exp(score × strength)` 只有在 score 有界时才是个可控的旋钮。
 * 困难档的评分是若干项之和，不设上限时任凭 strength 放大就会把分布压成一个点。
 *
 * ⚠️ **这个上限必须高于评分函数的实际取值范围，否则分层会被削平。**
 * 这里踩过一次：封路项定成 +900 之后，`关键封路格`（1850）与
 * `同行但不封死`（960）**双双被削到 900**，两者拿到完全相同的权重
 * （实测都是 42.96%），"封路优先"直接失效，而且不报错。
 *
 * 现在取 2400，高于 HostileScore 的实际上限（关键封路 +900、贴最大块 +700、
 * 挤拥 +60×4、断裂 +100×2、贴大块 +100×3、角 −80，约 2400）。
 * **改评分常数时必须回来核对这个值。**
 *
 * 饱和**不改变排序**，只压缩极端值 —— 前提是上限确实在取值范围之上。
 */
inline constexpr int kScoreSaturation = 2400;
...
/**
 * 覆盖权重的 strength 与加权占比。**仅供标定工具使用。**
 *
 * 偏置强度这类参数只有把"不同取值下的实际分布"并排打出来才能选，
 * 而每试一个值都重编译一次太慢。生产路径不调用它，所以
 * "同种子逐字节一致"的承诺不受影响。
 *
 * @param strength        指数放大强度（/100），例如 35 = 0.35。<=0 表示不改。
 * @param weighted_share  加权分支占比（/1000），例如 850 = 85%。越界表示不改。
 */
void SetWeightParamsForTesting(int strength, int weighted_share) noexcept;

/** 当前生效的困难档 strength 与加权占比（供工具回显）。 */
[[nodiscard]] int EffectiveHardWeightStrength() noexcept;
[[nodiscard]] int EffectiveHardWeightedShare() noexcept;
