...
//
// 参数依据（`seeds-v1.txt` 前 30 局、normal、**新生成规则**下实测）：
//
//   档位  配置              平均分   每局     2048   4096
//   入门  d4 /  300ms       37,742   2.6 s    80%    23%
//   标准  d6 /  800ms       42,043  15.1 s    97%    30%
//   最强  d8 / 2500ms       52,094  35.9 s    97%    50%
//
// **这三档是在"新生成规则"（P(4) 按难度 10/15/20%）下测的，
// 与 docs/results 里 2026-09 之前的旧数字不可比。**
//
// ⚠️ **depth 的单位是"树层"，不是"玩家步"**：max 与 chance 逐层交替，
// 所以 1 步前瞻 = 2 层（`stepsAhead` 做的就是 /2）。公开基准说的 "depth 8"
// 是 8 步 = 这里的 16 层，别把两者的数字直接比。
//
// ⚠️ depth 是**下限**，不是实际搜索深度：引擎的自适应深度会在空格少时
// 加深（实测约 41% 的步数触发），所以实际搜到的层数常常高于这里的值。
 ...
 */
const NOTES = {
  beginner: '最快，2048 达 80%',
  standard: '2048 达 97%，性价比最好',
  expert: '4096 达 50%，但每局约 36 秒',
};
