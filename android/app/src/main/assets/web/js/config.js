 ...
 *
 * ## `yieldFrame`：每步之间让出一帧（**手机上必须**）
 *
 * 用户反馈"手机上运行测试档会一卡一卡的，电脑上完全不一样"。实测（CPU 降速 4 倍
 * 模拟中低端手机、跑 40 步）：
 *
 * | 每步之间 | 页面重绘帧数 | 最长帧 |
 * |---|---|---|
 * | 不让出 | **0 帧** | 页面从头到尾一次都没刷新 |
 * | 让出一帧 | 40 帧 | 42ms |
 *
 * 原因是循环**一个接一个地占着主线程**：一步算完立刻发下一步，中间没有让出，
 * 浏览器根本没机会重绘。电脑快（每步几毫秒）看不出来，手机每步几十毫秒就卡住了。
 * 让出一帧只多花约 30% 时间（891ms → 1162ms），但画面能持续更新。
 *
 * ⚠️ 注意这只解决"渲染没机会跑"。如果引擎每步要算很久（例如强度选"最强"、
 * 每步 2.5 秒时间预算），那一段是 JNI 同步调用，照样会卡 ——
 * 那是"引擎太慢"，不是这个开关能解决的，跑测试请配低强度。
 */
export const SPEED = {
  ...
  test: { label: '测试', intervalMs: 0, instant: true, yieldFrame: true },
};
