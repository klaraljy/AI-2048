...
- **「难度」与「AI 强度」是两件不同的事，不得混用同一个词。**
  ...
  难度规格（用户确认，最高优先级）—— **2026-09-12 已修订，本条与旧版不同**：

  | 难度 | 位置分布 | 出 4 概率 |
  |---|---|---|
  | `easy` | **80% 按「安全分」加权 + 20% 全盘均匀** | 10% |
  | `normal` | 全盘均匀（标准 2048） | 15% |
  | `hard` | **75% 按「不利分」加权 + 25% 全盘均匀** | 20% |

  评分项（详见 `core/game.cpp` 的 SafeScore / HostileScore）：
  - 安全分 = 边缘 +150 / 角落 +250 / 空邻居 +80 / 已有邻居 −40
    （其中值是 2 的邻居再 +135、值是 4 再 +15）/ 贴 64+ 大块 −30
  - 不利分 = 拥挤度 +120（每个非空邻居）/ 贴大块 +80~+200（按 32/128/512 分档）
    / 断裂点 +150 / **可立即合并 −96 或 −24** / 角落 −80 / 边缘 −30
  ...
  而是实测标定的 **0.35 + 评分饱和 900 + 评分下界 −400**。原因：文档的评分常数
  加总后量级到 19.6，配 0.85 会让困难档的 max/min 权重比达到 **1700 万倍**，
  25% 的纯随机兜底被完全压倒，退化成"必定落同一格"。
  ...
  **根因**：评分函数末尾有一句 `return score < 0 ? 0 : score`，而一个空角落格
  本身**没有任何加分**（没有断裂点、不贴大块），得 0 分；减去 80 之后又被夹回 0。
