...
- **难度的前端与引擎实现必须逐位对拍。** 落点由 JS 决定（降级运行时）还是由
  ...
  #### ⚠️ 已知问题：2026-09-12 改规则后前端对拍未通过

  **引擎侧自洽且 93/93 测试通过，但前端与引擎还不逐字节一致**
  （`difficulty-parity.test.mjs` 三档各 25 局全部不一致）。
  前端已经实现新规则，但落点序列与引擎差一格，尚未定位。

  已经排除的：RNG 实现（JS 与 C++ 的 `NextBounded(1000)` 序列**完全相同**）、
  整数除法边界（JS 的 `Math.floor` 与 C++ 整除在 0..999 的映射上无差异）、
  权重表（已从 C++ 导出为常量表，不依赖 JS 的 `Math.exp`）。

  最小矛盾（可直接复现）：种子 1、`normal`、空盘，引擎前两块落在索引 **8 和 10**，
  而按源码逐行推算应是 **10 和 12**。运行时打印显示第二次生成取到
  `branch=900 cell=383 empty=15 idx=8`，但 `900` 是上一次生成的 value ——
  **每次生成的第 3 个随机数被下一次当成第 1 个用**，中间少了一次或多次消耗。

  **下一步手段（还没用过）**：在 `Rng::NextBounded` 里插一个全局计数器并打印，
  一次看清 `SpawnRandomTile` 到底取了几次、每次取到什么。这是唯一能一次定论的测量。

  > 定位过程本身也踩了坑，记下来：我先用"手工复刻同一逻辑"来验证，
  > 但手工版与引擎版**起点不同**（构造函数已经消耗过随机数），
  > 于是得出过"两者矛盾、不可能同时成立"的错误结论，白绕了几轮。
