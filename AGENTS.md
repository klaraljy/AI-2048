...
### ⛔ 重要性采样：三次独立测量全部为负，这条路已关闭（2026-09-13）

结论很硬 —— 在**任何**抽样规模、**任何**剪枝阈值下都打不过全展开：

| 场景 | 全展开 | 采样 | 结论 |
|---|---:|---:|---|
| 不限时，depth 6 | **43,862** | 35,384（4+4） | −19% |
| 限时 500ms，depth 8，种子 1–100 | 70,863（276 s） | 89,555（330 s，6+2） | ⚠️ 看似 +26%，**这是假胜** |
| 限时 500ms，depth 8，种子 101–300 | **96,529**（121 s） | 33,945 ~ 58,627（2+2 ~ 5+3） | **−39% ~ −65%** |

**"假胜"是怎么来的（这条教训比结论本身重要）**：
第一轮里"胜者"每局 52 s、基准每局 92 s —— **两者的实际计算量不同**。
根因是**胜负会改变游戏长度**：AI 打得越好、活得越久、每局越慢。
所以"每局耗时"**不是**算力对照量，用它当"同等算力"的依据会得出相反结论。

⚠️ **比较两套配置时，必须固定"每步预算"，不能用"每局耗时"当作可比性依据。**
第二轮换了一批种子后基准每局 121 s、采样只用 24 s，差距立刻暴露：
**基准 96,529 vs 采样 56,145**。

**为什么采样必然亏（不是实现细节问题）**：chance 节点末尾的 `total / weight_sum`
只累计**被选中**的分支，所以它算的是"选中子集的归一化平均"，**不是真实期望值的
无偏估计** —— 没被采样到的格子里的坏情况 AI 看不见，于是**系统性低估风险**。
要做对就得做成真正的重要性采样（按 1/选中概率 加权），那会引入方差。
投入产出不划算，已放弃。

### 💡 深度上限才是天花板：预算加到 2.5 秒是浪费（2026-09-13 实测）

「最强」档（depth 8）在 normal 难度下：

| 每步预算 | 平均分 | 每局 | 最深达到 |
|---:|---:|---:|---:|
| 500 ms | 70,863 | 92 s | 12 层 |
| 2500 ms | **70,863** | 97 s | **12 层** |

**两行数字完全相同。** 500 ms 时搜索就已经触到 `AdaptiveDepth` 给出的上限（12 层），
之后加多少预算都不会更深 —— 所有矩阵档位的「最深达到」也都是 12 层。

**所以想让 AI 更强，方向是抬高深度上限（或改变自适应策略），
而不是加时间预算。** 加预算只会在上限之下空转。

### 📊 当前基准（供以后对照）

「最强」档 = depth 8 / 2500 ms，`seeds-v1` 前 3 局、单线程、干净机器：

| 难度 | 平均分 | 平均步数 | 每局 | 2048 | 4096 | 8192 |
|---|---:|---:|---:|---:|---:|---:|
| normal | **70,863** | 3,182 | 96.7 s | 100% | 100% | 0% |
| hard | **59,712** | 2,595 | 108.4 s | 100% | 100% | 0% |

⚠️ 这两个数字是**单调性修复之后**的，与修复前（normal 110,686 / hard 无数据）
**不可比** —— 修复改变了单调性的量级尺度，键位饱和情况随之不同。
⚠️ 只有 3 局，标准误较大；困难档的 3 局分数极窄（59,152~60,104），
因为新规则强制封路让对局更确定。

### ⚠️ 单调性修复把历史权重全部标定作废了（2026-09-13）

**症状**：修复后 8192 到达率从 50% 掉到 **0%**（两个难度都是），分数也下降。

**原因不是"修复错了"，而是"权重必须与量级配套"**：修复改变了 `monotonicity`
的**绝对贡献尺度**，所以历史上扫出来的最优值（250）不再是最优点。

**实测决策影响力的变化（同一批局面）：**

| 项 | 修复前 | 修复后（mono=250） | mono=600 |
|---|---:|---:|---:|
| monotonicity | **77.3%** | 39.7% | **63.7%** |
| merge | 17.2% | 47.3% | 31.6% |
| corner | 8.4% | 21.2% | 14.2% |
| snake | 9.8% | 16.0% | 10.7% |
| **总分散布** | 6,765 | **3,814** | — |

⚠️ **注意总分散布是变小（6,765 → 3,814），不是变大。** 我一开始假设"修复抬高总分、
导致饱和削平"，方向搞反了 —— 后来靠实测纠正。**凡是"量级往哪个方向变"的推断，
都必须实测，不能靠推。**

**`mono` 权重的单峰扫描（6 局，depth 4）：**

| mono | 250 | 400 | **600** | 900 | 1200 | 1600 |
|---|---:|---:|---:|---:|---:|---:|
| 平均分 | 47,241 | 50,261 | **71,719** | 62,239 | 53,200 | 57,731 |
| 8192 | 0% | 0% | **17%** | 0% | 0% | 0% |

**单峰，峰值在 600 附近** —— 与修复前"最优 250、超过 320 变差"完全不同，
再次印证历史标定已作废。

**⚠️ 但 6 局的排序在对拍里大幅缩水，这是必须记住的教训：**

| 测量 | 样本 | mono=600 | mono=250 | 结论 |
|---|---:|---:|---:|---|
| 扫描 | 6 局 | 71,719 | 47,241 | +52% |
| 配对对拍 | 40 对 | 57,676 | 49,800 | +13.7%，**t = −1.51（分辨不出）** |

**差这么多不是测量错误**（空转自检通过：同配置对拍报"逐局完全相同"；
8 局自检均分 77,647 与扫描的 71,719 接近），而是**小样本排序极不可靠** ——
6 局的"单峰曲线"很可能是噪声形状。

**规则：6 局的扫描只能用来"筛掉明显差的档"，不能用来选最优值。**
选值必须用配对对拍，且要跑到分辨力足够（本例 40 对只能分辨 18%，
100 对约 11%）。

**这条与「决策影响力探针」的教训是同一类**：探针和短扫描都只能**发现异常**，
不能**指挥选值**。三者的可靠性排序：

```
配对对拍（能定值）  >  单峰扫描（只能筛档）  >  探针/影响力（只能发现异常）
```
...
### ✅ 决策：它是**语义修复**，判据是"是否更正确"，**不再回头验证**（用户 2026-09-13 确认）

实测（8 局绝对分对照）：38,297 → **49,091（+28%）**，平均步数 1,834 → **2,303**。

但**这条证据不够硬**：8 局的标准误约 ±9,000，方向一致但达不到配对对拍的标准。
之所以拿不到配对证据，是技术性的 —— 这个开关改的是**进程级全局行查表**，
`compare` 没法给 A/B 两侧设不同值（要做得把表改成按 (棋盘, 开关) 缓存两份）。

**用户裁定：这是语义修复、不是调参，判定标准是"是否更正确"而不是"是否统计显著"。**
理由充分，记在这里避免以后有人再来问它显不显著：

1. **空位不是"值为 0 的牌"** —— 旧行为在语义上就是错的，与 nneonneo / macroxue 不一致。
2. 它已通过 116 项单测 + 前端三档各 25 局**逐字节对拍**，**没有任何有害证据**。
3. 就算真实收益是 0，也不该回退 —— 回退等于把一个语义 bug 留着。

⚠️ **这条裁定只适用于"语义修复"这一类改动。** 调参（权重、阈值、深度）
仍然必须有配对对拍证据，`|t| < 2` 一律结论"分辨不出"、不许采纳。
两者的分界线是：**改的是"代码对不对"，还是"数值好不好"。**
