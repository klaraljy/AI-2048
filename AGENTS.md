...
- **「难度」与「AI 强度」是两件不同的事，不得混用同一个词。**
  ...
  而是实测标定的 **0.35 + 评分饱和 600**。原因：文档的评分常数加总后量级到 19.6，
  配 0.85 会让困难档的 max/min 权重比达到 **1700 万倍**，25% 的纯随机兜底
  被完全压倒，退化成"必定落同一格"。标定后是 1.9~5.4 倍，简单档落角率
  从 55% 降到 41%（均匀基准 26.7%）。

  ⚠️ **分数不跨难度比较。** 改的是生成规则，简单档分数天然更高。
  下面是**旧规则**的实测（`seeds-v1.txt` 各 30 局、深度 4）——
  **新规则下的基准尚未重跑**，这组数字只作历史参照：
