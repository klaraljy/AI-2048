...
```sh
# ES module 与 module worker 在 file:// 下会被拦，必须走 HTTP
python -m http.server 8080 --directory web
# 或者用引擎自带的 server（M4 之后）
engine\build\ai2048-server.exe --port 8765
```

前端**没有自动化测试**（无构建工具、无框架）。验收靠这份可重复的手工清单，
每次改动都要**实际走一遍**并把结果写进报告 —— 不许写"应该可以了"：

- [ ] 键盘四方向各走一次：方块平滑滑动到位，不停顿、不跳变
- [ ] 有一次合并发生：合并弹跳可见，得分浮字出现后消失
- [ ] 新生成的方块有出现动画，位置与逻辑格子一致
- [ ] 连续快速按方向键 10 次：无错位、无重影、无动画中断后的状态不一致
- [ ] 触屏（或浏览器设备模拟）滑动可操作；点击方向按钮可操作
- [ ] 音效在首次点击后能正常播放（autoplay 解锁生效）；静音开关有效
- [ ] 撤销后棋盘与分数都正确回退，且回退有视觉过渡
- [ ] 刷新页面后最高分仍在（localStorage 生效）
- [ ] 走到终局：遮罩出现，能重开；走到 2048：庆祝效果出现且不阻断继续
- [ ] 开一次 `prefers-reduced-motion`（浏览器模拟）后，动画被跳过而不是卡住
- [ ] 引擎未启动时打开页面：前端能降级运行（JS 基线）并给出可见提示，而不是白屏
