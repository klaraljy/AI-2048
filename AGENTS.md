...
（默认 `chance_sample_limit = 0`，但有效上限是 10）。实测：

| 档位 | 每步节点 | 每步耗时 |
|---|---|---|
| depth 6，跨步复用表，剪枝开 | 5,192 | 7.1 ms |
| depth 6，跨步复用表，剪枝关 | 12,481 | 20.2 ms |
| depth 8，跨步复用表（不限时） | 17,100 | 14.5 ms |
| depth 8，**每步清表** | — | 节点 +38% |
...
本机每局分数标准差约 15,000，分辨 5% 差异需要约 500 局。没有这个预算就别动。
...
标准误、配对 t 与符号检验 p 值。它的存在理由是一个已经量化的现实：

> 本机每局分数标准差约 **15,000**。100 局的均值标准误约 **1,500**，
> 所以 3~4% 的差异只有 0.7~1.1σ —— **用两次独立跑批的均值比大小，
> 在这个规模上根本分辨不出来**。
...
- 期望的样本量：要分辨 5% 的差异约需 **500 局**。跑不到就换成
  "结构上就对、可独立测量"的改动（节点数、命中率、深度），别去调权重。
...
单峰且峰很陡 —— **说明这里还有空间，但必须一次一格地扫**，
不能凭"更大更好"的直觉跳。

**工具：** `tools/calibration/` 下两个探针（`magnitude_probe` 看量级、
`influence_probe` 看决策影响力），几秒出结果，**不消耗对拍预算**。
改权重后先跑它们，再决定跑不跑对拍 —— 这比直接盲扫省几个数量级的时间。
